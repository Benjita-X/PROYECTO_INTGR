        ...
        <div class="container-lg">
            ...
            <div class="card shadow-sm rounded-3">
                ...
                <div class="card-header bg-body-tertiary border-bottom-0 pt-4">
                    <form method="GET" action="{{ route('director.historial') }}">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3 d-flex justify-content-start justify-content-md-end align-self-end gap-2">
                                ...
                                <a href="{{ route('director.historial') }}" class="btn btn-secondary">Limpiar</a>
                            </div>
                        </div>
                    </form>
                </div>
                            ...
                            <tbody>
                                @forelse ($casos as $caso)
                                    <tr>
                                        ...
                                        <td>{{ $caso->carrera }}</td>
                                        <td>
                                            {{-- Lógica de badge para Aceptado/Rechazado --}}
                                            <span class="badge rounded-pill fs-6
                                                @if($caso->estado == 'Aceptado') bg-success-subtle text-success-emphasis border border-success-subtle
                                                @elseif($caso->estado == 'Rechazado') bg-danger-subtle text-danger-emphasis border border-danger-subtle
                                        ...
                                        <td class="text-end pe-4">
                                            {{-- (Próximamente) Este botón llevará a la página de solo lectura --}}
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center p-4">
                                            <span class="text-muted">No se encontraron casos en el historial con los filtros seleccionados.</span>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
