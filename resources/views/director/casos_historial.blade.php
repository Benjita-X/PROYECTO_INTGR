                    ...
                    <form method="GET" action="{{ route('director.historial') }}">
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3 d-flex justify-content-start justify-content-md-end align-self-end gap-2">
                                ...
                                {{-- --- ¡BOTÓN AÑADIDO! --- --}}
                                <a href="{{ route('director.casos.rechazados.export', request()->query()) }}" class="btn btn-danger">
                            ...
                            <tbody>
                                @forelse ($casos as $caso)
                                    <tr>
                                        ...
                                        <td>{{ $caso->carrera }}</td>
                                        <td>
                                            <span class="badge rounded-pill fs-6 bg-danger-subtle text-danger-emphasis border border-danger-subtle">
                                                {{ ucfirst($caso->estado) }}
                                            </span>
                                        </td>
                                        <td>{{ $caso->created_at->translatedFormat('d M Y, H:i') }}</td>
                                        <td class="text-end pe-4">
                                            <a href="{{ route('director.casos.show', $caso) }}" class="btn btn-secondary btn-sm">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center p-4">
                                            <span class="text-muted">No se encontraron casos rechazados con los filtros seleccionados.</span>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
